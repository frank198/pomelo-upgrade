...
class Starter
{
	...
	static run(app, server, cb)
    {
		...
		if (utils.isLocal(server.host))
        {
			...
			_.forEach(server, (serverValue, key) =>
            {
				if (key === Constants.RESERVED.CPU)
                {
					cpus[server.id] = serverValue;
				}
	            options.push(util.format('%s=%s', key, serverValue));
			});
		...
		else
        {
			...
			_.forEach(server, (serverValue, key) =>
            {
				...
				cmd += util.format(' %s=%s ', key, serverValue);
			});
     ...
     *
     */
	static sshrun(cmd, host, cb = null)
    {
		let args = [];
		args.push(host);
