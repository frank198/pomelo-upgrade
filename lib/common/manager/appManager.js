...
const transactionLogger = require('pomelo-logger-upgrade').getLogger('transaction-log', __filename);
const transactionErrorLogger = require('pomelo-logger-upgrade').getLogger('transaction-error-log', __filename);
	...
	}, function(err)
	{
		...
		else
		{
			...
			process.nextTick(function()
			{
				...
				async.whilst(
					function() {return retry > 0 && flag;},
					function(callback)
					{
						let j = 0;
						retry--;
						async.forEachSeries(dmethods,
							function(method, cb)
							{
								method(cb);
					...
					function(err) {if (err) {logger.error('transaction process is executed with error: %j', err);}}
				);
			});
		}
	});
