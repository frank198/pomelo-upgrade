...
class BackendSessionService
{
     ...
     * @param  {String | Function} reason     kick reason
...
class BackendSessionUtility
{
	...
	static BackendSessionCB(service, cb, err, sinfo)
    {
        ...
        {
            // #getByUid
			_.forEach(sinfo, info =>
            {
				sessions.push(service.create(info));
			});
		}
		else
        {
...
{
	if (!(this instanceof BackendSessionService))
    {
		return new BackendSessionService(app);
	}
};