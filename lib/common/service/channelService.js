 ...
 * @constructor
 */
const ChannelService = function(app, opts)
{
	opts = opts || {};
	this.app = app;
	this.channels = {};
 ...
 */
const add = function(uid, sid, groups)
{
	if (!sid)
	{
 ...
 */
const deleteFrom = function(uid, sid, group)
{
	if (!uid || !sid || !group)
	{
		return false;
 ...
 * @api private
 */
const sendMessageByGroup = function(channelService, route, msg, groups, opts, cb)
{
...
const restoreChannel = function(self, cb)
{
	if (!self.store)
	{
		utils.invokeCallback(cb);
...
const addToStore = function(self, key, value)
{
	if (self.store)
	{
...
const removeFromStore = function(self, key, value)
{
	if (self.store)
	{
...
const loadAllFromStore = function(self, key, cb)
{
	if (self.store)
	{
...
const removeAllFromStore = function(self, key)
{
	if (self.store)
	{
...
const genKey = function(self, name)
{
	if (name)
	{
...
const genValue = function(sid, uid)
{
	return `${sid}:${uid}`;
};
