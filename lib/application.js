...
application.init = (opts) =>
{
	...
	logger.info('application inited: %j', application.getServerId());
};
 ...
 * @param  {Object|Function} component component instance or factory function of the component (单例 或者是 工厂模式的方法)
 * @param {*} opts (optional) construct parameters for the factory function
 * @returns {{}}
 */
application.load = (fileName, component, opts) =>
{
	if (!_.isString(fileName))
    {
		...
		component = fileName;
		fileName = null;
		if (_.isString(component.name) && !_.isFunction(component))
        {
	...
	if (!fileName && typeof _.isString(component.name))
    {
		fileName = component.name;
	}
 ...
 * @memberOf application
 */
application.route = (serverType, routeFunc) =>
{
...
application.stop = (force) =>
{
    ...
    {
		utils.invokeCallback(fun, application, shutDown, cancelShutDownTimer);
	}
	else
    {
		shutDown();
 ...
 */
application.set = (setting, value = null, attach = false) =>
{
 ...
 * @memberOf application
 */
application.registerAdmin = (moduleId, module = null, opts = null) =>
{
 ...
 * @memberOf application
 */
application.getServerById = serverId =>
{
...
application.getServerFromConfig = serverId =>
{
 ...
 * @memberOf application
 */
application.getServersByType = serverType =>
{
 ...
 * @memberOf application
 */
application.addServers = servers =>
{
	...
	_.forEach(servers, (server, serverId) =>
	{
		...
		slist = application.serverTypeMaps[server.serverType];
		if (!slist)
        {
			application.serverTypeMaps[server.serverType] = slist = [];
		}
		ApplicationUtility.ReplaceServer(slist, server);

        // update global server type list
		if (application.serverTypes.indexOf(server.serverType) < 0)
        {
 ...
 * @memberOf application
 */
application.replaceServers = servers =>
{
	if (!servers)
    {
		return;
	...
	_.forEach(servers, (server, serverId) =>
    {
		...
		const slist = application.serverTypeMaps[serverType];
		if (!slist)
        {
