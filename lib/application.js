    ...
    if (Boolean(realPath) && Boolean(reload))
    {
        fs.watch(realPath, (event) =>
        {
            if (event === 'change')
            {
                ...
                this.loadConfigBaseApp(key, val, false);
            }
        });
    }
};
...
Application.beforeStopHook = function(fun)
{
    ...
    if (Boolean(fun) && typeof fun === 'function')
    {
    ...
    appUtil.startByType(this, () =>
    {
        ...
        const startUp = () =>
        {
            appUtil.optComponents(this.loaded, Constants.RESERVED.START, (err) =>
            {
                ...
                {
                    // 某一个服务器启动完成
                    logger.info(`${this.getServerType()} ${this.getServerId()} 开始调用启动后的函数...`, );
                    this.afterStart(cb);
                }
            });
        };
...
Application.afterStart = function(cb)
{
    ...
    appUtil.optComponents(this.loaded, Constants.RESERVED.AFTER_START, (err) =>
    {
        ...
        logger.info(`${serverType} ${id} startup in ${usedTime} ms`);
...
Application.removeServers = function(ids)
{
    ...
    this.event.emit(events.REMOVE_SERVERS, removeServers);
};
...
/**
 * Update crons from current application at runtime.
 *
 * @param  {Array} crons old crons would be removed in application
 * @memberOf Application
 */
Application.UpdateeCrons = function(crons)
{
    if (!crons || !crons.length)
    {
        logger.warn('ids is not defined.');
...
const replaceServer = function(slist, serverInfo)
{
...
const removeServer = function(slist, id)
{
...
const contains = function(str, settings)
{
    if (!settings)
    {
        return false;
...
const bindEvents = function(Event, app)
{
...
const addFilter = function(app, type, filter)
{
