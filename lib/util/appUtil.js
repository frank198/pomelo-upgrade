...
const processArgs = (app, args) =>
{
	...
	const serverId = args.id || app.Master.id;
...
const configLogger = (app) =>
{
	if (process.env.POMELO_LOGGER !== 'off')
    {
		...
		const originPath = path.join(app.Base, Constants.FILEPATH.LOG);
		const presentPath = path.join(app.Base, Constants.FILEPATH.CONFIG_DIR, env, path.basename(Constants.FILEPATH.LOG));
...
{
	const filePath = path.join(app.Base, Constants.FILEPATH.SERVER_DIR, app.serverType, Constants.FILEPATH.LIFECYCLE);
