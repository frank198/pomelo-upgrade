...
class ConnectorUtility
{
	...
	static BindEvents(connector, socket)
	{
		...
		if (connector.connection && maxConnections)
		{
		...
		socket.on('message', msg =>
		{
			...
			if (connector.useAsyncCoder)
            {
				return ConnectorUtility.HandleMessageAsync(self, msg, session, socket);
			}
	...
	static HandleMessageAsync(connector, msg, session, socket)
	{
		if (connector.decode)
        {
			connector.decode(msg, session, (err, dmsg) =>
			{
				if (err)
                {
				...
				ConnectorUtility.DoHandleMessage(connector, dmsg, session);
			});
		}
		else if (connector.connector.decode)
        {
			connector.connector.decode(msg, socket, (err, dmsg) =>
			{
				if (err)
                {
					logger.error('fail to decode message from client %s .', err.stack);
	...
	static DoHandleMessage(connector, msg, session)
	{
		if (!msg)
        {
            // discard invalid message
		...
		if (connector.useCrypto)
        {
			const verified = ConnectorUtility.VerifyMessage(connector, session, msg);
			if (!verified)
            {
				logger.error('fail to verify the data received from client.');
				return;
		...
		ConnectorUtility.HandleMessage(connector, session, msg);
	}
